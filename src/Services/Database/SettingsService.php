...
namespace AfterPay\Services\Database;
...
class SettingsService extends DatabaseBaseService
{
    ...
    public function loadSetting($webstore, $mode,$country=0)
    {
        $setting = $this->getValues(Settings::class, ['name', 'webstore','country'], [$mode, $webstore,$country], ['=','=','=']);
    ...
    public function loadSettings($settingType)
    {
        ...
        if(is_array($results))
        {
            foreach ($results as $item)
            {
                ...
                {
                    $settings[] = ['PID_'.$item->webstore.'_'.$item->country => $item->value];
                }
            }
        }
        return $settings;
    ...
    public function saveSettings($mode, $settings)
    {
        if($settings)
        {
            foreach ($settings as $setting)
            {
                ...
                {
                    $id = 0;
                    $temp = explode('_',$store);
                    $country = $temp[2];
                    $store = (int)str_replace('PID_', '',  $temp[1]);

                    if($store > 0)
                    {
                        $existValue = $this->getValues(Settings::class, ['name', 'webstore','country'], [$mode, $store,$country], ['=','=','=']);
